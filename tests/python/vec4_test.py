...
@pytest.mark.parametrize("Class", [(m3d.Vector4f), (m3d.Vector4d)])
def test_vec4_attrs(Class: Vector4Cls):
...
class TestVec4Constructors:
    def test_default_constructor(self, Class: Vector4Cls) -> None:
        # Default constructor initializes to (0.0, 0.0, 0.0, 0.0)
        vec = Class()
        assert (vec.x == 0.0) and (vec.y == 0.0) and (vec.z == 0.0) and (vec.w == 0.0)

    def test_single_arg_constructor(self, Class: Vector4Cls) -> None:
        # Single scalar argument x initializes to (x, x, x, x)
        vec = Class(1.0)
        assert (vec.x == 1.0) and (vec.y == 1.0) and (vec.z == 1.0) and (vec.w == 1.0)

    def test_double_args_constructor(self, Class: Vector4Cls) -> None:
        # Two scalar arguments x, y initializes to (x, y, y, y)
        vec = Class(1.0, 2.0)
        assert (vec.x == 1.0) and (vec.y == 2.0) and (vec.z == 2.0) and (vec.w == 2.0)

    def test_triple_args_constructor(self, Class: Vector4Cls) -> None:
        # Three scalar arguments x, y, z initializes to (x, y, z, z)
        vec = Class(1.0, 2.0, 3.0)
        assert (vec.x == 1.0) and (vec.y == 2.0) and (vec.z == 3.0) and (vec.w == 3.0)

    def test_full_args_constructor(self, Class: Vector4Cls) -> None:
        # Three scalar arguments x, y, z, w, initializes to (v, y, z, w)
        vec = Class(1.0, 2.0, 3.0, 4.0)
        assert (vec.x == 1.0) and (vec.y == 2.0) and (vec.z == 3.0) and (vec.w == 4.0)


@pytest.mark.parametrize(
    "Class,FloatType", [(m3d.Vector4f, np.float32), (m3d.Vector4d, np.float64)]
)
def test_numpy_array_constructor(Class: Vector4Cls, FloatType):
    # From a numpy array (requires same dtype)
    vec_np = np.array([19.0, 23.0, 29.0, 31.0])
    vec = Class(vec_np.astype(FloatType))
    assert (vec.x == 19.0) and (vec.y == 23.0) and (vec.z == 29.0) and (vec.w == 31.0)

    vec = Class(vec_np.reshape(1, 4).astype(FloatType))
    assert (vec.x == 19.0) and (vec.y == 23.0) and (vec.z == 29.0) and (vec.w == 31.0)

    vec = Class(vec_np.reshape(4, 1).astype(FloatType))
    assert (vec.x == 19.0) and (vec.y == 23.0) and (vec.z == 29.0) and (vec.w == 31.0)


@pytest.mark.parametrize("Class", [(m3d.Vector4f), (m3d.Vector4d)])
def test_vec4_accessors(Class: Vector4Cls) -> None:
    # Can get x, y, z, and w as properties
    vec = Class(3.0, 7.0, 11.0, 13.0)
    assert (vec.x == 3.0) and (vec.y == 7.0) and (vec.z == 11.0) and (vec.w == 13.0)
    # Can get elements using __getitem__ as well
    assert (vec[0] == 3.0) and (vec[1] == 7.0) and (vec[2] == 11.0) and (vec[3] == 13.0)
...
class TestVec4Operators:
    def test_comparison_operator(self, Class: Vector4Cls) -> None:
        vec_a = Class(1.0, 2.0, 3.0, 4.0)
        vec_b = Class(1.0, 2.0, 3.0, 4.0)
    ...
    def test_vector_addition(self, Class: Vector4Cls) -> None:
        vec_a = Class(3.0, 7.0, 11.0, 13.0)
        vec_b = Class(4.0, 5.0, 6.0, 7.0)
        vec_sum = vec_a + vec_b
        assert (
            (vec_sum.x == 7.0)
            and (vec_sum.y == 12.0)
    ...
    def test_vector_substraction(self, Class: Vector4Cls) -> None:
        vec_a = Class(3.0, 7.0, 11.0, 13.0)
        vec_b = Class(4.0, 5.0, 6.0, 7.0)
    ...
    def test_vector_scalar_product(self, Class: Vector4Cls) -> None:
        vec = Class(4.0, 8.0, 12.0, 16.0)
        scale = 3.5
    ...
    def test_unary_negative(self, Class: Vector4Cls) -> None:
        vec = Class(3.0, 5.0, 7.0, 11.0)
        vec_neg = -vec
        assert (
            (vec_neg.x == -3.0)
            and (vec_neg.y == -5.0)
...
class TestVec4Methods:
    def test_length(self, Class: Vector4Cls) -> None:
        epsilon = 1e-2
    ...
    def test_normalize_in_place(self, Class: Vector4Cls) -> None:
        EPSILON = 1e-5
        x, y, z, w = 1.0, 2.0, 3.0, 4.0
    ...
    def test_normalize(self, Class: Vector4Cls) -> None:
        EPSILON = 1e-5
        x, y, z, w = 1.0, 2.0, 3.0, 4.0
        norm = np.sqrt(x**2 + y**2 + z**2 + w**2)
        vec = Class(x, y, z, w)
    ...
    def test_dot_product(self, Class: Vector4Cls) -> None:
