...
def test_vec2_attrs(Class: Vector2Cls) -> None:
...
class TestVec2Constructors:
    def test_default_constructor(self, Class: Vector2Cls) -> None:
        # Default constructor initializes to (0.0, 0.0)
        vec = Class()
        assert (vec.x == 0.0) and (vec.y == 0.0)

    def test_single_arg_constructor(self, Class: Vector2Cls) -> None:
        # Single scalar argument x_val initializes to (x_val, x_val)
        vec = Class(1.0)
        assert (vec.x == 1.0) and (vec.y == 1.0)

    def test_full_args_constructor(self, Class: Vector2Cls) -> None:
...
@pytest.mark.parametrize("Class", [(m3d.Vector2f), (m3d.Vector2d)])
def test_vec2_accessors(Class: Vector2Cls) -> None:
...
class TestVec2Operators:
    def test_comparison_operator(self, Class: Vector2Cls) -> None:
        vec_a = Class(1.0, 2.0)
        vec_b = Class(1.0, 2.0)
    ...
    def test_vector_addition(self, Class: Vector2Cls) -> None:
        vec_a = Class(3.0, 7.0)
        vec_b = Class(4.0, 5.0)
        vec_sum = vec_a + vec_b
        assert (vec_sum.x == 7.0) and (vec_sum.y == 12.0)

    def test_vector_substraction(self, Class: Vector2Cls) -> None:
        vec_a = Class(3.0, 7.0)
        vec_b = Class(4.0, 5.0)
        vec_diff = vec_a - vec_b
        assert (vec_diff.x == -1.0) and (vec_diff.y == 2.0)

    def test_vector_scalar_product(self, Class: Vector2Cls) -> None:
        vec = Class(4.0, 8.0)
        scale = 3.5
    ...
    def test_unary_negative(self, Class: Vector2Cls) -> None:
        vec = Class(3.0, 5.0)
        vec_neg = -vec
